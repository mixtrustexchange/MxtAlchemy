...
import { fastForwardTo, getTime } from '@alchemyio/core-utils/utils/hardhat/rpc';
...
describe('DecayTokenModule', () => {
  ...
  before('set decay rate', async () => {
    ...
    await TokenModule.connect(owner).initialize('Alchemy Network Token', 'alchemy', decimal);
    await tx.wait();
  });
  ...
  describe('When attempting to initialize it again', () => {
    it('reverts', async () => {
      await assertRevert(
        TokenModule.initialize('Alchemy Network Token Updated', 'alchemy', decimal),
        'AlreadyInitialized()'
      );
    });
  });
  ...
  describe('Before minting any tokens', () => {
    ...
    it('the constructor arguments are set correctly', async () => {
      assert.equal(await TokenModule.name(), 'Alchemy Network Token');
      assert.equal(await TokenModule.symbol(), 'alchemy');
