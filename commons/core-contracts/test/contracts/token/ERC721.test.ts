...
describe('ERC721', function () {
  ...
  before('deploy the contract', async function () {
    ...
    const tx = await ERC721.initialize('Alchemy NFT', 'alchemyNFT', '');
    await tx.wait();
  });
  ...
  describe('When attempting to initialize it again', function () {
    it('reverts', async function () {
      await assertRevert(
        ERC721.initialize('Alchemy NFT', 'alchemyNFT', 'http://alchemy.io'),
        'AlreadyInitialized()'
      );
    });
  });
  ...
  describe('Before minting any NFT', function () {
    it('the constructor arguments are set correctly', async function () {
      assert.equal(await ERC721.name(), 'Alchemy NFT');
      assert.equal(await ERC721.symbol(), 'alchemyNFT');
    });
