...
import { findEvent } from '@alchemyio/core-utils/utils/ethers/events';
...
describe('ERC20', function () {
  ...
  before('deploy the contract', async function () {
    ...
    const tx = await ERC20.initialize('Alchemy Network Token', 'alchemy', 18);
    await tx.wait();
  });
  ...
  describe('When attempting to initialize it again', function () {
    it('reverts', async function () {
      await assertRevert(
        ERC20.initialize('Alchemy Network Token Updated', 'alchemy', 18),
        'AlreadyInitialized()'
      );
    });
  });
  ...
  describe('Before minting any tokens', function () {
    ...
    it('the constructor arguments are set correctly', async function () {
      assert.equal(await ERC20.name(), 'Alchemy Network Token');
      assert.equal(await ERC20.symbol(), 'alchemy');
