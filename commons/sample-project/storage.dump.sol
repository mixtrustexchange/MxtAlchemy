...
contract GlobalStorage {
    bytes32 private constant _SLOT_GLOBAL_STORAGE = keccak256(abi.encode("io.alchemy.sample-project.Global"));
    struct GlobalStore {
        uint value;
        uint someValue;
    }
...
contract InitializableStorage {
    bytes32 private constant _SLOT_INITIALIZABLE_STORAGE = keccak256(abi.encode("io.alchemy.sample-project.Initializable"));
...
contract SettingsStorage {
    bytes32 private constant _SLOT_SETTINGS_STORAGE = keccak256(abi.encode("io.alchemy.sample-project.Settings"));
