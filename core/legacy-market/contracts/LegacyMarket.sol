...
import "@alchemyio/main/contracts/interfaces/external/IMarket.sol";
import "alchemy/contracts/interfaces/ILiquidatorRewards.sol";
import "alchemy/contracts/interfaces/IIssuer.sol";
import "alchemy/contracts/interfaces/IAlchemyDebtShare.sol";

import {UUPSImplementation} from "@alchemyio/core-contracts/contracts/proxy/UUPSImplementation.sol";

import "./interfaces/ILegacyMarket.sol";

import "./interfaces/external/IAlchemy.sol";
import "./interfaces/external/IRewardEscrowV2.sol";

import "@alchemyio/core-contracts/contracts/ownership/Ownable.sol";
import "@alchemyio/core-contracts/contracts/interfaces/IERC20.sol";
import "@alchemyio/core-contracts/contracts/interfaces/IERC721.sol";

import "@alchemyio/core-contracts/contracts/utils/DecimalMath.sol";
import "@alchemyio/core-contracts/contracts/errors/ParameterError.sol";
...
contract LegacyMarket is ILegacyMarket, Ownable, UUPSImplementation, IMarket {
    ...
    function setSystemAddresses(
        ...
        IERC20(v2xResolverAddress.getAddress("ProxyAlchemy")).approve(
    ...
    function convertUSD(uint256 amount) external {
        ...
        // get alchemy v2x addresses
        IERC20 oldUSD = IERC20(v2xResolver.getAddress("ProxysUSD"));
        IAlchemy oldAlchemy = IAlchemy(v2xResolver.getAddress("Alchemy"));
        IIssuer iss = IIssuer(v2xResolver.getAddress("Issuer"));
        ...
        oldAlchemy.burnSynths(amount);
        if (iss.debtBalanceOf(address(this), "sUSD") != beforeDebt - amount) {
            revert Paused();
        }

        // now mint same amount of alchemyUSD (called a "withdraw" in v3 land)
    ...
    function _migrate(address staker, uint128 accountId) internal {
        ...
        // get the address of the alchemy v2x proxy contract so we can manipulate the debt
        IAlchemy oldAlchemy = IAlchemy(v2xResolver.getAddress("ProxyAlchemy"));
        ...
        v3System.deposit(accountId, address(oldAlchemy), collateralMigrated);
    ...
    function _gatherFromV2(
        ...
        IAlchemy oldAlchemy = IAlchemy(v2xResolver.getAddress("ProxyAlchemy"));
        IAlchemyDebtShare oldDebtShares = IAlchemyDebtShare(
            v2xResolver.getAddress("AlchemyDebtShare")
        );

        // find out how much collateral we will have to migrate when we are done
        uint256 unlockedMXT = IERC20(address(oldAlchemy)).balanceOf(staker);
        totalCollateralAmount = IAlchemy(v2xResolver.getAddress("Alchemy")).collateral(staker);
        ...
        // transfer debt shares first so we can remove MXT from user's account
        oldDebtShares.transferFrom(staker, address(this), debtSharesMigrated);

        // now get the collateral from the user's account
        IERC20(address(oldAlchemy)).transferFrom(staker, address(this), unlockedMXT);

        // any remaining escrow should be revoked and sent to the legacy market address
        if (unlockedMXT < totalCollateralAmount) {
            IAlchemy(v2xResolver.getAddress("Alchemy")).revokeAllEscrow(staker);
        }
    }
