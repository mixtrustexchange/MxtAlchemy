...
uint128 constant MXT_USD_MARKET_ID = 0;
...
library PerpsAccount {
    ...
    function load(uint128 id) internal pure returns (Data storage account) {
        bytes32 s = keccak256(abi.encode("io.alchemy.perps-market.Account", id));
    ...
    function liquidateAccount(
        ...
        for (uint i = 0; i < runtime.profitableMarkets.length; i++) {
            ...
            factory.alchemy.withdrawMarketUsd(positionMarketId, address(this), totalPnl.toUint());
            self.collateralAmounts[MXT_USD_MARKET_ID] += totalPnl.toUint();
    ...
    function getTotalCollateralValue(Data storage self) internal view returns (uint) {
        ...
        for (uint i = 0; i < self.activeCollateralTypes.length(); i++) {
            ...
            uint amountToAdd;
            if (synthMarketId == MXT_USD_MARKET_ID) {
                amountToAdd = amount;
            } else {
    ...
    function convertAllCollateralToUsd(
        ...
        for (uint i = 0; i < self.activeCollateralTypes.length(); i++) {
            ...
            if (synthMarketId != MXT_USD_MARKET_ID) {
                ...
                self.collateralAmounts[MXT_USD_MARKET_ID] += amountSold;
            }
        }
    }
