...
import "@alchemyio/core-modules/contracts/modules/AssociatedSystemsModule.sol";
import "@alchemyio/core-contracts/contracts/initializable/InitializableMixin.sol";
...
import "@alchemyio/core-modules/contracts/storage/FeatureFlag.sol";
import "@alchemyio/core-contracts/contracts/interfaces/IERC165.sol";
...
import "@alchemyio/core-contracts/contracts/errors/AddressError.sol";
...
contract PerpsMarketFactoryModule is IPerpsMarketFactoryModule {
     ...
     */
    function setAlchemy(IAlchemySystem alchemy) external override {
        OwnableStorage.onlyOwner();
        PerpsMarketFactory.Data storage store = PerpsMarketFactory.load();

        store.alchemy = alchemy;
        (address usdTokenAddress, ) = alchemy.getAssociatedSystem("USDToken");
        store.usdToken = ITokenModule(usdTokenAddress);
        store.oracle = alchemy.getOracleManager();
    }
    ...
    function createMarket(
        ...
        PerpsMarketFactory.Data storage store = PerpsMarketFactory.load();
        uint128 perpsMarketId = store.alchemy.registerMarket(address(this));
