...
// @custom:artifact @alchemyio/core-modules/contracts/storage/DecayToken.sol:DecayToken
library DecayToken {
    bytes32 private constant _SLOT_DECAY_TOKEN_STORAGE = keccak256(abi.encode("io.alchemy.core-modules.DecayToken"));
    struct Data {
        uint256 decayRate;
        uint256 epochStart;
...
library AsyncOrder {
    ...
    function load(uint128 marketId) internal pure returns (Data storage store) {
        bytes32 s = keccak256(abi.encode("io.alchemy.spot-market.AsyncOrder", marketId));
        assembly {
            store.slot := s
        }
    }
...
library AsyncOrderClaim {
    ...
    function load(uint128 marketId, uint256 claimId) internal pure returns (Data storage store) {
        bytes32 s = keccak256(abi.encode("io.alchemy.spot-market.AsyncOrderClaim", marketId, claimId));
        assembly {
            store.slot := s
        }
    }
...
library AsyncOrderConfiguration {
    ...
    function load(uint128 marketId) internal pure returns (Data storage asyncOrderConfiguration) {
        bytes32 s = keccak256(abi.encode("io.alchemy.spot-market.AsyncOrderConfiguration", marketId));
        assembly {
...
library Price {
    ...
    function load(uint128 marketId) internal pure returns (Data storage price) {
        bytes32 s = keccak256(abi.encode("io.alchemy.spot-market.Price", marketId));
        assembly {
            price.slot := s
        }
    }
...
library SpotMarketFactory {
    bytes32 private constant _SLOT_SPOT_MARKET_FACTORY = keccak256(abi.encode("io.alchemy.spot-market.SpotMarketFactory"));
    struct Data {
        address usdToken;
        address oracle;
        address alchemy;
...
library Wrapper {
    ...
    function load(uint128 marketId) internal pure returns (Data storage wrapper) {
        bytes32 s = keccak256(abi.encode("io.alchemy.spot-market.Wrapper", marketId));
        assembly {
            wrapper.slot := s
        }
    }
