...
This system generates synths (ERC-20 tokens) and markets that exchange them with stablecoins generated by [Alchemy](../../protocol/alchemy) at a price determined by the [oracle manager](../../protocol/oracle-manager) after applying fees.

To execute orders, the markets withdraw or deposit stablecoins with Alchemy (negatively or positively impacting the position of liquidity providers to the market, respectively). The market also reports the total supply of the synths multiplied by their current price to Alchemy as debt, such that the synths are collateralized by liquidity providers in Alchemy. This amount is also locked, meaning that liquidity providers cannot remove delegated credit such that the issued synths would become undercollateralized.

## Creating Synths

To create a new synth and a corresponding market, call `createSynth(string memory tokenName, string memory tokenSymbol, address marketOwner)`. This function returns a market ID, which can be used in a pool's configuration within Alchemy to provide
...
- **Commit**
  ...
  - A new [AsyncOrderClaim](https://github.com/Alchemyio/alchemy-v3/blob/main/markets/spot-market/contracts/storage/AsyncOrderClaim.sol) is created and the trader is set as the owner.
- **Settle**
...
Markets which are able to [provide collateral directly to Alchemy](../../protocol/alchemy/contracts/modules/core/MarketCollateralModule.sol) can issue synths of equivalent value after applying fees. The relevant logic is included in the [Wrapper Module](./contracts/modules/WrapperModule.sol).
...
```
configured skew scale: 1000 alchemyETH
before fill synth balance in market: 100 alchemyETH

--> user buys 10 alchemyETH using buyExactOut
