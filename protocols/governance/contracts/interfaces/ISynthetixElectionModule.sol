...
interface IAlchemyElectionModule is IBaseElectionModule {
    ...
    /// @notice Sets the Alchemy v2 DebtShare contract that determines vote power
    function setDebtShareContract(address newDebtShareContractAddress) external;

    /// @notice Returns the Alchemy v2 DebtShare contract that determines vote power
    function getDebtShareContract() external view returns (address);

    /// @notice Sets the Alchemy v2 DebtShare snapshot that determines vote power for this epoch
    function setDebtShareSnapshotId(uint snapshotId) external;

    /// @notice Returns the Alchemy v2 DebtShare snapshot id set for this epoch
    function getDebtShareSnapshotId() external view returns (uint);

    /// @notice Returns the Alchemy v2 debt share for the provided address, at this epoch's snapshot
    ...
    /// @notice Allows users to declare their Alchemy v2 debt shares on other chains
    ...
    /// @notice Returns the Alchemy v2 debt shares for the provided address, at this epoch's snapshot, in other chains
