...
/**
 * @title Module for managing the alchemyUSD token as an associated system.
 */
interface IUSDTokenModule is ITokenModule {
    /**
     * @notice Allows the core system to burn alchemyUSD held by the `from` address, provided that it has given allowance to `spender`.
     * @param from The address that holds the alchemyUSD to be burned.
     * @param spender The address to which the holder has given allowance to.
     * @param amount The amount of alchemyUSD to be burned, denominated with 18 decimals of precision.
     */
