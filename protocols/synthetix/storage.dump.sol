...
// @custom:artifact @alchemyio/core-contracts/contracts/ownership/OwnableStorage.sol:OwnableStorage
library OwnableStorage {
    bytes32 private constant _SLOT_OWNABLE_STORAGE = keccak256(abi.encode("io.alchemy.core-contracts.Ownable"));
    struct Data {
        address owner;
...
// @custom:artifact @alchemyio/core-contracts/contracts/proxy/ProxyStorage.sol:ProxyStorage
contract ProxyStorage {
    bytes32 private constant _SLOT_PROXY_STORAGE = keccak256(abi.encode("io.alchemy.core-contracts.Proxy"));
    struct ProxyStore {
...
// @custom:artifact @alchemyio/core-contracts/contracts/token/ERC20Storage.sol:ERC20Storage
library ERC20Storage {
    bytes32 private constant _SLOT_ERC20_STORAGE = keccak256(abi.encode("io.alchemy.core-contracts.ERC20"));
    struct Data {
        string name;
        string symbol;
        uint8 decimals;
...
// @custom:artifact @alchemyio/core-contracts/contracts/token/ERC721EnumerableStorage.sol:ERC721EnumerableStorage
library ERC721EnumerableStorage {
    bytes32 private constant _SLOT_ERC721_ENUMERABLE_STORAGE = keccak256(abi.encode("io.alchemy.core-contracts.ERC721Enumerable"));
    struct Data {
...
// @custom:artifact @alchemyio/core-contracts/contracts/token/ERC721Storage.sol:ERC721Storage
library ERC721Storage {
    bytes32 private constant _SLOT_ERC721_STORAGE = keccak256(abi.encode("io.alchemy.core-contracts.ERC721"));
    struct Data {
        string name;
        string symbol;
        string baseTokenURI;
...
// @custom:artifact @alchemyio/core-contracts/contracts/utils/DecimalMath.sol:DecimalMath
library DecimalMath {
...
// @custom:artifact @alchemyio/core-contracts/contracts/utils/HeapUtil.sol:HeapUtil
library HeapUtil {
...
// @custom:artifact @alchemyio/core-contracts/contracts/utils/SetUtil.sol:SetUtil
library SetUtil {
    struct UintSet {
        Bytes32Set raw;
    }
...
// @custom:artifact @alchemyio/core-modules/contracts/modules/NftModule.sol:NftModule
contract NftModule {
    bytes32 internal constant _INITIALIZED_NAME = "NftModule";
}

// @custom:artifact @alchemyio/core-modules/contracts/storage/AssociatedSystem.sol:AssociatedSystem
library AssociatedSystem {
    ...
    function load(bytes32 id) internal pure returns (Data storage store) {
        bytes32 s = keccak256(abi.encode("io.alchemy.core-modules.AssociatedSystem", id));
        assembly {
            store.slot := s
        }
    }
...
// @custom:artifact @alchemyio/core-modules/contracts/storage/FeatureFlag.sol:FeatureFlag
library FeatureFlag {
    struct Data {
        bytes32 name;
        bool allowAll;
    ...
    function load(bytes32 featureName) internal pure returns (Data storage store) {
        bytes32 s = keccak256(abi.encode("io.alchemy.core-modules.FeatureFlag", featureName));
        assembly {
            store.slot := s
        }
    }
...
// @custom:artifact @alchemyio/core-modules/contracts/storage/Initialized.sol:Initialized
library Initialized {
    struct Data {
        bool initialized;
    }
    function load(bytes32 id) internal pure returns (Data storage store) {
        bytes32 s = keccak256(abi.encode("io.alchemy.code-modules.Initialized", id));
        assembly {
            store.slot := s
        }
    }
...
// @custom:artifact @alchemyio/oracle-manager/contracts/storage/NodeDefinition.sol:NodeDefinition
library NodeDefinition {
    ...
    function load(bytes32 id) internal pure returns (Data storage node) {
        bytes32 s = keccak256(abi.encode("io.alchemy.oracle-manager.Node", id));
        assembly {
            node.slot := s
        }
    }
...
// @custom:artifact @alchemyio/oracle-manager/contracts/storage/NodeOutput.sol:NodeOutput
library NodeOutput {
    struct Data {
        int256 price;
        uint256 timestamp;
...
library Account {
    ...
    function load(uint128 id) internal pure returns (Data storage account) {
        bytes32 s = keccak256(abi.encode("io.alchemy.alchemy.Account", id));
        assembly {
            account.slot := s
        }
    }
...
library CollateralConfiguration {
    bytes32 private constant _SLOT_AVAILABLE_COLLATERALS = keccak256(abi.encode("io.alchemy.alchemy.CollateralConfiguration_availableCollaterals"));
    struct Data {
    ...
    function load(address token) internal pure returns (Data storage collateralConfiguration) {
        bytes32 s = keccak256(abi.encode("io.alchemy.alchemy.CollateralConfiguration", token));
        assembly {
...
library Market {
    ...
    function load(uint128 id) internal pure returns (Data storage market) {
        bytes32 s = keccak256(abi.encode("io.alchemy.alchemy.Market", id));
        assembly {
            market.slot := s
        }
    }
...
library MarketCreator {
    bytes32 private constant _SLOT_MARKET_CREATOR = keccak256(abi.encode("io.alchemy.alchemy.Markets"));
    struct Data {
...
library OracleManager {
    bytes32 private constant _SLOT_ORACLE_MANAGER = keccak256(abi.encode("io.alchemy.alchemy.OracleManager"));
    struct Data {
...
library Pool {
    ...
    function load(uint128 id) internal pure returns (Data storage pool) {
        bytes32 s = keccak256(abi.encode("io.alchemy.alchemy.Pool", id));
        assembly {
            pool.slot := s
        }
    }
...
library SystemPoolConfiguration {
    bytes32 private constant _SLOT_SYSTEM_POOL_CONFIGURATION = keccak256(abi.encode("io.alchemy.alchemy.SystemPoolConfiguration"));
    struct Data {
